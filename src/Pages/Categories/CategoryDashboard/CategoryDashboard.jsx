...
const CategoryDashboard = () => {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`https://zaporka-backend.onrender.com/api/v1/categories/${deleteId}`, {
        method: 'DELETE',
        headers: {
          'accept': '*/*',
  ...
  const handleEdit = async (id) => {
    try {
      ...
      const response = await fetch(`https://zaporka-backend.onrender.com/api/v1/categories/${id}`, {
        method: 'PUT',
        headers: {
          'accept': '*/*',
          'Authorization': `Bearer ${token}`,
