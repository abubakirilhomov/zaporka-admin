...
const CategoryDashboard = () => {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/api/v1/categories/${deleteId}`,
        {
          method: "DELETE",
          headers: {
            accept: "*/*",
  ...
  const columns = [
    {
      key: "index",
      label: "#",
      render: (value, row, i) => indexOfFirst + i + 1,
    },
    {
      key: "name",
      label: "Название",
  ...
  return (
    <div className="sm:p-6 max-w-[98%] bg-base-200">
      ...
      <CustomTable
        ...
        actions={actions.filter(
          (a) => !a.showIf || currentItems.some((row) => a.showIf(row))
        )}
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
          <div className="bg-base-100 p-6 rounded shadow-lg space-y-4 w-full max-w-sm">
            <h2 className="text-lg font-bold text-error-content">
              Удалить категорию?
            </h2>
            <p className="text-sm">
              Это действие нельзя отменить. При удалении категории также будут
              удалены все связанные с ней продукты.
            </p>
